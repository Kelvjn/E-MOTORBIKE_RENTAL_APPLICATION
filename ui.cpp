...
void UI::RQ(const struct Motorbike& motorbike ){
    UserInfo renterInfo;
    User* currentUser = auth->getCurrentUser();
    renterInfo.userRating = currentUser->rating;
    renterInfo.userCreditPoints = currentUser->creditPoints;
    renterInfo.hasValidLicense = hasValidLicense(currentUser->username);
    ...
    while (true) {
            ...
            cin >> startData;
            if (bookingManager->validateDateFormat(startData)) {
            break;
        }
        cout << "Invalid date format. Please use DD/MM/YYYY format.\n";
    }
        ...
        cout << "Invalid date format. Please use DD/MM/YYYY format.\n";
           ...
           double totalCP= bookingManager->calculatingRentalCost(motorbike,startData,endDate);
        
            if (( renterInfo.userCreditPoints - totalCP )< 0){
                cout<<"Not enough CP";
                return ;
                pauseScreen();
                
            }
            /*if (bookingManager->isReting(username)){
                cout<<"you already rent a moto";
                pauseScreen();
                return;
            }*/
            ...
            cout << left<< setw(22) << (startData + "-" + endDate)
                        <<  setw(12) << motorbike.brand
                        << setw(12) << motorbike.model
                        << setw(10) << motorbike.color
                        << setw(8)  << motorbike.size
                        << setw(15) << motorbike.plateNo
                        << setw(12) << motorbike.ownerUsername
                        << setw(6)  << totalCP
                        << "\n";
            cout << "Are you sure you want to rent this motorbike? (y/n): ";
        char confirm;
        cin >> confirm;
    
        if (confirm == 'y' || confirm == 'Y') {
            bookingManager->creatingBooking(username,motorbike,startData,endDate);
         cout<<"Booking sucessfully!"<<endl;
        } else {
...
void UI::myBooking() {
    string username = auth->getCurrentUser()->username;
    vector<Booking> rentalRequests = bookingManager->getUserRentalRequests(username);
   cout << "Found " << rentalRequests.size() << " rental resquest\n\n";
    for (size_t i = 0; i < rentalRequests.size(); i++) {
        const Booking& request = rentalRequests[i];
        cout <<"[" << (i + 1) << "] "<< request.bookingId << " | "
                 << request.renterUsername << " | "
                 << request.startDate << "-" << request.endDate << " | "
                 << request.brand << " " << request.model << " | "
                 << request.status << "\n";
    }
    cout << "Enter rental request number to approve the request (0 to return): ";
    int choice;
    cin >> choice;
    
    if (choice > 0 && choice <= (int)rentalRequests.size()) {
        acceptRequest(rentalRequests[choice - 1]);
    }
}
void UI::acceptRequest(const struct Booking& request){
    vector<User> allUsers = auth->getAllUsers();
    for (User& user : allUsers) {
        if (user.username == request.renterUsername) {
            user.creditPoints = request.totalCost - user.creditPoints;
            
        }
    }
    vector<Motorbike> allMotorbikes = bookingManager->getAllMotorbikes();
    for (Motorbike& motobike : allMotorbikes) {
        if (motobike.brand==request.brand && motobike.model==request.model && 
            motobike.color == request.color && motobike.ownerUsername == request.ownerUsername){
                motobike.isAvailable = false;
                
            }
    }
     bookingManager->approveBooking(request.bookingId,request.ownerUsername);
