...
void BookingManager::loadReviews() {
    ...
    while (getline(file, line)) {
        ...
        try {
            ...
            getline(ss, review.reviewId, '|');
            getline(ss, review.motorbikeId, '|');
            getline(ss, review.reviewerUsername, '|');
            
            string temp;
            getline(ss, temp, '|');
            if (!temp.empty()) {
                review.rating = stod(temp);
            } else {
                review.rating = 0.0;
            }
            
            getline(ss, review.comment, '|');
            getline(ss, review.reviewDate, '|');
            
            reviews.push_back(review);
        } catch (const exception& e) {
            cout << "Warning: Skipping malformed review data line\n";
            continue;
        }
    }
...
void BookingManager::saveReviews() {
    ...
    // Write header for admin readability
    file << "# Review Data Format: reviewId|motorbikeId|reviewerUsername|rating|comment|reviewDate\n";
...
vector<Motorbike> BookingManager::searchMotorbikes(const SearchCriteria& criteria) {
    vector<Motorbike> availableMotorbikes = getAvailableMotorbikes();
    return filterMotorbikes(availableMotorbikes, criteria);
}

vector<Motorbike> BookingManager::filterMotorbikes(const vector<Motorbike>& motorbikes, const SearchCriteria& criteria) {
...
bool BookingManager::addReview(const string& motorbikeId, const string& reviewerUsername, 
                              double rating, const string& comment) {
    Review review;
    review.reviewId = generateReviewId();
    review.motorbikeId = motorbikeId;
    review.reviewerUsername = reviewerUsername;
    ...
    char dateBuffer[11]; // DD/MM/YYYY format
    strftime(dateBuffer, sizeof(dateBuffer), "%d/%m/%Y", currentTime);
    review.reviewDate = string(dateBuffer);
    
    reviews.push_back(review);
    saveReviews();
    
    // Update motorbike average rating
    updateMotorbikeRating(motorbikeId);
    
    return true;
}

vector<Review> BookingManager::getMotorbikeReviews(const string& motorbikeId) {
    vector<Review> motorbikeReviews;
    for (const Review& review : reviews) {
        if (review.motorbikeId == motorbikeId) {
            motorbikeReviews.push_back(review);
        }
    }
    return motorbikeReviews;
}

double BookingManager::getMotorbikeAverageRating(const string& motorbikeId) {
    vector<Review> motorbikeReviews = getMotorbikeReviews(motorbikeId);
    if (motorbikeReviews.empty()) {
        return 0.0;
    }
    
    double totalRating = 0.0;
    for (const Review& review : motorbikeReviews) {
        totalRating += review.rating;
    }
    
    return totalRating / motorbikeReviews.size();
}

void BookingManager::updateMotorbikeRating(const string& motorbikeId) {
    Motorbike* motorbike = getMotorbikeById(motorbikeId);
    if (motorbike) {
        motorbike->rating = getMotorbikeAverageRating(motorbikeId);
        saveMotorbikes();
    }
}
...
void BookingManager::createSampleReviews() {
    // Review for MB1
    Review review1;
    review1.reviewId = "RV1";
    review1.motorbikeId = "MB1";
    review1.reviewerUsername = "admin";
    review1.rating = 4.5;
    review1.comment = "Great electric scooter! Very quiet and eco-friendly. Perfect for city commuting.";
    review1.reviewDate = "10/11/2025";
    reviews.push_back(review1);
    
    // Review for MB1 (second review)
    Review review2;
    review2.reviewId = "RV2";
    review2.motorbikeId = "MB1";
    review2.reviewerUsername = "Thinh";
    review2.rating = 4.5;
    review2.comment = "Smooth ride, good battery life. Highly recommended!";
    review2.reviewDate = "15/11/2025";
    reviews.push_back(review2);
    
    // Review for MB2
    Review review3;
    review3.reviewId = "RV3";
    review3.motorbikeId = "MB2";
    review3.reviewerUsername = "Thinh";
    review3.rating = 4.0;
    review3.comment = "Reliable Honda as always. Good fuel economy and comfortable seat.";
    review3.reviewDate = "20/11/2025";
    reviews.push_back(review3);
    
    // Review for MB3
    Review review4;
    review4.reviewId = "RV4";
    review4.motorbikeId = "MB3";
    review4.reviewerUsername = "admin";
    review4.rating = 5.0;
    review4.comment = "Amazing performance! Fast and responsive. Love this bike!";
    review4.reviewDate = "25/11/2025";
    reviews.push_back(review4);
    
    // Review for MB4
    Review review5;
    review5.reviewId = "RV5";
    review5.motorbikeId = "MB4";
    review5.reviewerUsername = "admin";
    review5.rating = 3.8;
    review5.comment = "Perfect for beginners. Easy to handle and park.";
    review5.reviewDate = "01/12/2025";
    reviews.push_back(review5);
    
    saveReviews();
    
    // Update motorbike ratings based on reviews
    updateMotorbikeRating("MB1");
    updateMotorbikeRating("MB2");
    updateMotorbikeRating("MB3");
    updateMotorbikeRating("MB4");
}
...
vector<Review> BookingManager::getAllReviews() {
    return reviews;
}
